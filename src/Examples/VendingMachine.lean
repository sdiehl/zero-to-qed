...
inductive Product where
  | TokenTreats | HallucinaSnacks | PromptCola | ContextCrunch | SafetyFirst
  deriving Repr, DecidableEq

def Product.price : Product → Nat
  | .TokenTreats => 150 | .HallucinaSnacks => 100 | .PromptCola => 125
  | .ContextCrunch => 175 | .SafetyFirst => 200

def Product.name : Product → String
  | .TokenTreats => "Token Treats" | .HallucinaSnacks => "Hallucina-Snacks"
...
theorem insert_return (coin : Nat) : (returnChange (insertCoin coin empty)).1 = coin := rfl

def exampleTransaction : Product × Nat := Id.run do
  let m := empty
  let m := insertDollar m
  let m := insertDollar m
  let (snack, m) := vend .HallucinaSnacks m (by omega)
...
def main : IO Unit := do
  ...
  for p in [Product.TokenTreats, .HallucinaSnacks, .PromptCola, .ContextCrunch, .SafetyFirst] do
    IO.println s!"  {p.name}: {formatPrice p.price}"
  IO.println ""
  let m := empty
  let m := insertDollar m
  let m := insertDollar m
  let (product, m) := vend .HallucinaSnacks m (by omega)
  let (change, _) := returnChange m
